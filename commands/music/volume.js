...
module.exports = class ChangeVolumeCommand extends Command {
    constructor(client) {
        super(client, {
            ...
            group: 'music',
            memberName: 'volume',
            description: '`AL: moderate` Changes the volume.',
            format: '[level]',
            details: 'The volume level ranges from 0-10. You may specify "up" or "down" to modify the volume level by 2.', // eslint-disable-line max-len
    ...
    run(msg, args) {
        ...
        if (!queue) {
            return msg.embed({
                ...
                description: `${msg.author}, there isn't any music playing to change the volume of. Better queue some up!` // eslint-disable-line max-len
            });
        }
        if (!args) {
            return msg.embed({
