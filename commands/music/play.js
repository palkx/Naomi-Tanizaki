...
const { DEFAULT_VOLUME, GOOGLE_API, MAX_LENGTH, MAX_SONGS, PASSES, SOUNDCLOUD_API } = process.env; // eslint-disable-line max-len
    ...
    async run(msg, args) {
        ...
        if (!queue) {
            ...
            if (!permissions.has('CONNECT')) {
                return msg.embed({
                    color: _sdata.colors.red,
                    description: `${msg.author}, I don't have permission to join your voice channel. No parties allowed there.` // eslint-disable-line max-len
                });
            }
            if (!permissions.has('SPEAK')) {
                return msg.embed({
                    color: _sdata.colors.red,
                    description: `${msg.author}, I don't have permission to speak in your voice channel. What a disappointment.` // eslint-disable-line max-len
                });
            }
