...
module.exports = class UnlockCommand extends Command {
	...
	run(msg, args) {
		...
		if (channel.type !== 'text') {
			return msg.embed({ color: colors.blue, description: `${msg.author}, you can only unlock text channels.` });
		}

		const channelLocks = this.client.provider.get(msg.guild.id, 'locks', []);
		if (!channelLocks.includes(channel.id)) {
			return msg.embed({ color: colors.blue, description: `${msg.author}, this channel is not locked.` });
		}
