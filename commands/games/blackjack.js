    ...
    run(msg, { bet }) {
     ...
     && !noHit) { // eslint-disable-line no-unmodified-loop-condition
            ...
            const embed = {
                ...
                fields: [],
                footer: {
                    text: blackjack.cardsRemaining()
                        ? `Cards remaining: ${blackjack.cardsRemaining()}`
                        : `Shuffling`
                }
            };
                    ...
                    fields: [
                        {
                            ...
                            value: stripIndents`
								...
								Value: ${Blackjack.isSoft(currentHand)
                                     ? 'Soft ' // eslint-disable-line indent
                                     : ''}${Blackjack.handValue(currentHand)}`, // eslint-disable-line indent
                            inline: true
                        },
                        {
                            name: '**Dealer hand**',
                            value: stripIndents`
								${dealerHand[0]} - XX
						 		Value: ${Blackjack.isSoft([dealerHand[0]])
                                     ? 'Soft ' // eslint-disable-line indent
                                     : ''}${Blackjack.handValue([dealerHand[0]])}`, // eslint-disable-line indent
                            inline: true
                        }
                    ],
                    footer: {
