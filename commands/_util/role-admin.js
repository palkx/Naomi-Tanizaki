...
module.exports = class RoleWhitelistCommand extends Command {
	...
	async run (msg, { job, aMember, role }) { // eslint-disable-line consistent-return, require-await
		...
		if (!_role) {
			return msg.embed({ color: _sdata.colors.red, description: `${role} group is not exist on server` });
		}
		...
		if (_job) {
			...
			return msg.embed({
				...
				description: `${aMember} has been added to the ${_role} admins.`
			});
		} else {
			if (!roleAdminList.includes(aMember.id)) {
			...
			return msg.embed({
				color: _sdata.colors.green,
				description: `${aMember} has been removed from the ${_role} admins.`
			});
		}
	}
};
